    ...
    <div className='header sticky top-0 z-50 flex items-center justify-between px-4 md:px-8 py-4 shadow-md bg-gradient-to-r from-white to-gray-50'>
      ...
      <div className={`nav-items fixed top-0 right-0 h-screen w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out rounded-l-2xl lg:static lg:h-auto lg:w-auto lg:bg-transparent lg:shadow-none lg:transform-none ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'}`}>
        ...
        <ul className='flex flex-col lg:flex-row items-start lg:items-center pt-16 lg:pt-0 px-6 lg:px-0 space-y-4 lg:space-y-0 lg:space-x-6 xl:space-x-10 text-base lg:text-lg font-semibold text-gray-700'>
          ...
          <li><NavLink to="/" className={({ isActive }) => `block py-2 px-3 duration-200 ${isActive ? 'text-amber-500' : 'text-gray-700'} hover:text-amber-500`} onClick={() => setMobileMenuOpen(false)}>Home</NavLink></li>
          <li><NavLink to="/about" className={({ isActive }) => `block py-2 px-3 duration-200 ${isActive ? 'text-amber-500' : 'text-gray-700'} hover:text-amber-500`} onClick={() => setMobileMenuOpen(false)}>About</NavLink></li>
          <li><NavLink to="/contact" className={({ isActive }) => `block py-2 px-3 duration-200 ${isActive ? 'text-amber-500' : 'text-gray-700'} hover:text-amber-500`} onClick={() => setMobileMenuOpen(false)}>Contact Us</NavLink></li>
          <li>
            <NavLink to="/cart" className={({ isActive }) => `relative inline-flex items-center py-2 px-3 duration-200 ${isActive ? 'text-amber-500' : 'text-gray-700'} hover:text-amber-500`} onClick={() => setMobileMenuOpen(false)}>
              Cart
              {cartItems?.length > 0 && (
          ...
          <li><NavLink to="/grocery" className={({ isActive }) => `block py-2 px-3 duration-200 ${isActive ? 'text-amber-500' : 'text-gray-700'} hover:text-amber-500`} onClick={() => setMobileMenuOpen(false)}>Grocery</NavLink></li>
          ...
          {/* More button dropdown for desktop */}
...
{showDropdown && user && (
  <div className="absolute right-0 mt-2 w-80 bg-white border border-gray-200 rounded-xl shadow-lg z-50 text-gray-800 p-6 text-center">
    ...
    {user.photoURL ? (
      ...
      <UserCircle className="w-20 h-20 text-gray-500 mx-auto mb-4" />
    )}
